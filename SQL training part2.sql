...
use store;
select * from products; -- Select all fields
...
select * from products where (unit_price>2 and quantity_in_stock>60) or quantity_in_stock<20;
select * from products where unit_price>2 and (quantity_in_stock>60 or quantity_in_stock<20); -- inside the parentheses is checked first ( Uses parentheses to prioritize conditions)
...
select * from orders where order_date>='2018-01-01' and order_date<='2018-12-30';-- or we can write
select * from orders where order_date between '2018-01-01' and '2018-12-30'; 
--  "LIKE" clause to filter records in the "orders" table
...
select * from customers where last_name regexp 'mac|son|ase'; -- can be anywhere in last name 
...
select * from customers where last_name regexp 'mac|son$|ase'; 
-- The dollar sign ($) at the end of "son" indicates that the match should occur at the end of the "last_name" value.
